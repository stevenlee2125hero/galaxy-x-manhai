...
import {
  AlertTriangle, ArrowLeft, BookOpen, Bookmark, Check, ChevronLeft, ChevronRight,
  CircleDollarSign, CloudDownload, Compass, ExternalLink, EyeOff, FileArchive, FolderDown, Globe2,
  Heart, Home as HomeIcon, LockKeyhole, LogOut, Search, ShieldCheck,
...
type BaiduStatus = { configured:boolean; connected:boolean; scope?:string|null; mode?:string };
...
function matchingBaiduFile(comic:Comic,files:BaiduFile[]){
  ...
  return files.find(file=>!file.isdir&&((title.length>1&&normalizeTitle(file.name).includes(title))||(subtitle.length>3&&normalizeTitle(file.name).includes(subtitle))));
}
...
export default function Home(){
  ...
  const [syncMessage,setSyncMessage]=useState("每天 01:07 检查目录");
  ...
  useEffect(()=>{
    ...
    queueMicrotask(()=>{ setFavorites(savedFavorites); setBlocked(savedBlocked); setHistory(savedHistory); setWeeklyReads(currentWeekly); setMemberships(savedMemberships); setLocked(sessionLocked); });
    fetch("/comic-catalog.json").then(r=>r.json()).then((data:Catalog)=>{const comics=Array.isArray(data.comics)?data.comics:[];setCatalog(comics);setCatalogMeta({count:data.count||comics.length,curatedCount:data.curatedCount||Math.min(54,comics.length),networkCount:data.networkCount||Math.max(0,comics.length-54)});}).catch(()=>setToast("目录加载失败，请稍后刷新"));
    ...
    fetch("/sync-status.json").then(r=>r.json()).then(data=>data.lastRun&&setSyncMessage(`${data.message||"目录已同步"} · 每天 01:07`)).catch(()=>undefined);
    listOfflinePacks().then(setOfflinePacks).catch(()=>undefined);
    fetch("/api/baidu/status",{cache:"no-store"}).then(r=>r.json()).then((status:BaiduStatus)=>{setBaiduStatus(status);if(status.connected){const dir=localStorage.getItem("galaxy:baidu-dir")||"/";fetch(`/api/baidu/files?dir=${encodeURIComponent(dir)}`,{cache:"no-store"}).then(r=>r.json()).then(data=>Array.isArray(data.items)&&setBaiduFiles(data.items)).catch(()=>undefined);}}).catch(()=>undefined);
  ...
  const toggleFavorite=(comic:Comic)=>{
    const next=favorites.includes(comic.id)?favorites.filter(id=>id!==comic.id):[comic.id,...favorites];
    persistList("galaxy:favorites",next,setFavorites);
    setToast(next.includes(comic.id)?"已收藏，将优先检查更新":"已取消收藏");
  };
  const blockComic=(comic:Comic)=>{
    persistList("galaxy:blocked",[comic.id,...blocked.filter(id=>id!==comic.id)],setBlocked);
    persistList("galaxy:favorites",favorites.filter(id=>id!==comic.id),setFavorites);
    setSelected(null); setToast("已屏蔽，不再推荐或检查更新");
  };
  ...
  const loadBaiduFiles=async(directory:string)=>{
    ...
    const response=await fetch(`/api/baidu/files?dir=${encodeURIComponent(dir)}`); const data=await response.json();
    if(!response.ok){setToast(data.error||"读取百度网盘失败");return;}
    setBaiduFiles(data.items||[]); setToast(`已读取 ${data.items?.length||0} 个漫画文件/目录`);
  };
  ...
  const openPreferred=(comic:Comic)=>{
    ...
    const preferred=orderedSources(comic,accessPolicies,memberships).find(source=>sourceTier(source,accessPolicies,memberships)!=="risk");
      ...
      : tab==="sources"
        ...
        : <>
          ...
          {activeCategory==="本周排行"?<>
            ...
            {catalog.length===0?<section className="loading-grid">{Array.from({length:8}).map((_,index)=><i key={index}/>)}</section>:<WeeklyRankings boards={weeklyBoards} weeklyReads={weeklyReads} favorites={favorites} history={history} getStatuses={comic=>comicStatuses(comic,accessPolicies,memberships,offlinePacks.some(pack=>pack.comicId===comic.id),Boolean(matchingBaiduFile(comic,baiduFiles)))} onOpen={openPreferred} onDetails={setSelected} onFavorite={toggleFavorite}/>} 
          </>:<>
            ...
            {catalog.length===0?<section className="loading-grid">{Array.from({length:8}).map((_,index)=><i key={index}/>)}</section>:<section className="cover-grid">{shownComics.map((comic,index)=><ComicCard key={comic.id} comic={comic} favorite={favorites.includes(comic.id)} chapter={history[comic.id]} statuses={comicStatuses(comic,accessPolicies,memberships,offlinePacks.some(pack=>pack.comicId===comic.id),Boolean(matchingBaiduFile(comic,baiduFiles)))} weeklyReads={weeklyReads[comic.id]||0} priority={index<4} onOpen={()=>openPreferred(comic)} onDetails={()=>setSelected(comic)} onFavorite={()=>toggleFavorite(comic)}/>)}</section>}
...
function LockScreen({onEnter}:{onEnter:()=>void}){
  ...
  return <main className="lock-screen">
    ...
    <p className="eyebrow">PRIVATE MOBILE LIBRARY</p><h1>清爽地，进入漫画世界</h1><p className="lock-note">为 iPhone 15 Plus 优化 · 登录状态保留 30 天</p>
...
function InstallPrompt(){
  ...
  return <section className="install-prompt">
    <span className="install-icon"><img src="/app-icon-192.png" alt="" aria-hidden="true"/></span>
    <div><p>IPHONE APP</p><strong>安装到 iPhone 15 Plus</strong><small>全屏打开 · 保留登录与阅读记录</small></div>
    <button onClick={()=>setShowSteps(value=>!value)}><Smartphone size={15}/>{showSteps?"收起":"安装方法"}</button>
    {showSteps&&<ol><li><Share2 size={14}/><span>在 Safari 底部点“分享”</span></li><li><span className="install-plus">＋</span><span>选择“添加到主屏幕”</span></li><li><Check size={14}/><span>右上角点“添加”即可</span></li></ol>}
  </section>;
}
...
function ComicCard({comic,favorite,chapter,statuses,rank,weeklyReads,priority,onOpen,onDetails,onFavorite}:{comic:Comic;favorite:boolean;chapter?:number;statuses:DisplayTier[];rank?:number;weeklyReads:number;priority:boolean;onOpen:()=>void;onDetails:()=>void;onFavorite:()=>void}){
  return <article className="comic-card">
    <div className="poster-stage"><button className="poster-card" onClick={onOpen} aria-label={`打开${comic.title}`}>
    ...
    <div className="card-caption"><button className="caption-details" onClick={onDetails}><strong>{comic.title}</strong><small>{rank?`本周打开阅读 ${weeklyReads} 次`:comic.catalogOnly?"真实网络目录 · 正文待核验":comic.latest?`更新至 ${comic.latest} 话`:"章节以官方目录为准"}</small></button><button className={favorite?"active":""} onClick={onFavorite} aria-label={favorite?"取消收藏":"收藏"}><Heart size={17} fill={favorite?"currentColor":"none"}/></button></div>
  </article>;
}

function WeeklyRankings({boards,weeklyReads,favorites,history,getStatuses,onOpen,onDetails,onFavorite}:{boards:WeeklyBoard[];weeklyReads:Record<string,number>;favorites:string[];history:HistoryMap;getStatuses:(comic:Comic)=>DisplayTier[];onOpen:(comic:Comic)=>void;onDetails:(comic:Comic)=>void;onFavorite:(comic:Comic)=>void}){
  return <section className="weekly-rankings">{boards.map((board,boardIndex)=><section className="weekly-board" key={board.key}>
    ...
    {board.comics.length?<div className="weekly-rail">{board.comics.map((comic,index)=><ComicCard key={`${board.key}-${comic.id}`} comic={comic} favorite={favorites.includes(comic.id)} chapter={history[comic.id]} statuses={getStatuses(comic)} rank={index+1} weeklyReads={weeklyReads[comic.id]||0} priority={boardIndex===0&&index<4} onOpen={()=>onOpen(comic)} onDetails={()=>onDetails(comic)} onFavorite={()=>onFavorite(comic)}/>)}</div>:<div className="weekly-empty"><CloudDownload size={18}/><span>{board.key==="netdisk"?"连接百度网盘并匹配漫画文件后生成网盘榜":"本周还没有符合条件的实际阅读记录"}</span></div>}
  </section>)}</section>;
}
...
function DetailSheet({comic,allComics,favorite,historyChapter,packs,baiduFile,policies,memberships,titleStatus,onClose,onRead,onChapters,onFavorite,onBlock,onImport,onOpenPack,onToggleMembership,onSelect}:{comic:Comic;allComics:Comic[];favorite:boolean;historyChapter?:number;packs:OfflinePack[];baiduFile?:BaiduFile;policies:Record<string,SourcePolicy>;memberships:string[];titleStatus:Record<string,TitleStatus>;onClose:()=>void;onRead:()=>void;onChapters:()=>void;onFavorite:()=>void;onBlock:()=>void;onImport:(files:File[])=>void;onOpenPack:(pack:OfflinePack)=>void;onToggleMembership:(name:string)=>void;onSelect:(comic:Comic)=>void}){
  ...
  return <div className="sheet-backdrop" onClick={onClose}><section className="detail-sheet" onClick={event=>event.stopPropagation()}>
    ...
    <div className="detail-actions"><button className={favorite?"active":""} onClick={onFavorite}><Heart size={17} fill={favorite?"currentColor":"none"}/>{favorite?"已收藏":"收藏"}</button><button onClick={()=>inputRef.current?.click()}><CloudDownload size={17}/>缓存正文</button><button className="danger" onClick={onBlock}><EyeOff size={17}/>不喜欢</button></div>
...
function Profile({favorites,blocked,catalog,sources,packs,policies,memberships,baiduStatus,baiduFiles,onRefreshBaidu,onLoadBaidu,onBaiduFilesChange,onToggleMembership,onImport,onDelete,onOpen,onRestore,onLock}:{favorites:number;blocked:string[];catalog:Comic[];sources:number;packs:OfflinePack[];policies:Record<string,SourcePolicy>;memberships:string[];baiduStatus:BaiduStatus;baiduFiles:BaiduFile[];onRefreshBaidu:()=>Promise<BaiduStatus>;onLoadBaidu:(directory:string)=>Promise<void>;onBaiduFilesChange:(files:BaiduFile[])=>void;onToggleMembership:(name:string)=>void;onImport:(files:File[])=>void;onDelete:(id:string)=>void;onOpen:(pack:OfflinePack)=>void;onRestore:(id:string)=>void;onLock:()=>void}){
  ...
  return <section className="profile-page">
    ...
    <p className="membership-note">受浏览器隐私限制，本站不能读取其他平台的登录 Cookie。你成功登录、购买或开通后，在这里确认一次即可保存在本机；下次优先打开该来源。平台自身登录有效期仍由平台决定，网络和风险状态每天 01:07 复检。</p>
    ...
    <div className="settings-card"><div><span>真实漫画目录</span><strong>{catalog.length} 部</strong></div><div><span>目录与风险</span><strong>每天 01:07 检查</strong></div><div><span>网络优先级</span><strong>直接 → 登录 → 付费/会员 → 域外</strong></div><div><span>风险来源</span><strong>自动隔离，不作为入口</strong></div></div>
    <button className="logout-button" onClick={onLock}><LogOut size={17}/>退出登录</button>
  </section>;
}

function BaiduPanel({status,files,onRefresh,onLoadFiles,onFilesChange}:{status:BaiduStatus;files:BaiduFile[];onRefresh:()=>Promise<BaiduStatus>;onLoadFiles:(directory:string)=>Promise<void>;onFilesChange:(files:BaiduFile[])=>void}){
  ...
  return <section className="baidu-panel">
    <div className="baidu-head"><span className="baidu-logo">度</span><div><p>BAIDU NETDISK</p><h3>百度网盘漫画库</h3><small>{status.connected?"登录状态已记住 · 已载入 "+files.length+" 项":status.configured?"等待百度授权":"尚未配置官方授权"}</small></div><em className={status.connected?"connected":""}>{status.connected?"网盘看":"未连接"}</em></div>
    ...
    {status.connected&&<div className="baidu-browser"><label><span>漫画目录</span><input value={directory} onChange={event=>setDirectory(event.target.value)} placeholder="/漫画"/><button onClick={()=>onLoadFiles(directory)}><Search size={15}/>读取</button></label><div>{files.slice(0,18).map(file=>file.isdir?<button className="baidu-file" key={file.fs_id} onClick={()=>{setDirectory(file.path);void onLoadFiles(file.path);}}><FolderDown size={17}/><span>{file.name}<small>文件夹</small></span><ChevronRight size={15}/></button>:<a className="baidu-file" key={file.fs_id} href={"/api/baidu/download?fs_id="+encodeURIComponent(file.fs_id)} target="_blank" rel="noopener noreferrer"><FileArchive size={17}/><span>{file.name}<small>{formatSize(file.size)}</small></span><ExternalLink size={15}/></a>)}</div><button className="baidu-muted" disabled={busy} onClick={disconnect}>断开百度网盘</button><p>只读取你授权的目录索引；授权令牌会被记住并自动续期，不会公开分享、删除或上传网盘文件。</p></div>}
